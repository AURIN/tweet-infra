    ...
    .initConfig({
      ...
      http: {
        ...
        delgeoindex: {
          options: {
            url: "http://" + grunt.sensitiveConfig.couchdb.authadmin + "@"
            + grunt.option("masterip") + ":5984/" + grunt.option("database") + "/_index/indexes/json/geo-index",
            method: "delete"
          },
          "dest": "/tmp/a.log"
        },
        addgeoindex: {
          options: {
            url: "http://" + grunt.sensitiveConfig.couchdb.authadmin + "@"
            + grunt.option("masterip") + ":5984/" + grunt.option("database") + "/_index",
            method: "post",
            headers: {
              "Content-Type": "application/json"
            },
            ...
            "  \"ddoc\": \"indexes\"," +
            "    \"index\": {" +
            "    \"fields\": [\"created_time\", \"coordinates.coordinates\"]" +
            "  }," +
            "  \"name\": \"geo-index\"," +
            "    \"type\": \"json\"" +
            "}"
        ...
        querygeo: {
          options: {
            url: "http://" + grunt.sensitiveConfig.couchdb.authadmin + "@"
            + grunt.option("masterip") + ":5984/" + grunt.option("database") + "/_find",
            method: "post",
            qs: {stable: false, update: false},
            headers: {
              "Content-Type": "application/json"
            },
            body: "{" +
            "   \"fields\" : [\"_id\", \"coordinates\", \"user.full_name\", \"created_time\"]" +
            "   ,\"selector\": {" +
            "      \"$and\": [" +
            "        {\"coordinates.coordinates\": {\"$gt\": [14, 121]}}" +
            "       ,{\"coordinates.coordinates\": {\"$lt\": [15, 122]}}" +
            "       ,{\"created_time\": {\"$gt\": \"1371896160\"}}" +
            "      ]" +
            "   }" +
            "}"
          },
        ...
        explaingeo: {
          options: {
            url: "http://" + grunt.sensitiveConfig.couchdb.authadmin + "@"
            + grunt.option("masterip") + ":5984/" + grunt.option("database") + "/_explain",
            method: "post",
            qs: {stable: false, update: false},
            headers: {
              "Content-Type": "application/json"
            ...
            "   \"selector\": {" +
            "      \"$and\": [" +
           "        {\"coordinates.coordinates\": {\"$gt\": [15, 124]}}" +
            "       {\"coordinates.coordinates\": {\"$gt\": [16, 128]}}" +
            "       ,{\"created_time\": {\"$gt\": \"1371896160\"}}" +
            "      ]" +
            "   }" +
            "}"
